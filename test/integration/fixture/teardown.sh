...
docker container run --rm -v $PWD/test/integration/fixture/terraform:/app -w /app --entrypoint '' --link localstack_main --env TF_VAR_HOST=localstack_main --network "$CURRENT_DIR"_integration_test_network hashicorp/terraform:light terraform destroy -lock=false -auto-approve
